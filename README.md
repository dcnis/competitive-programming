...
### 3) de.schmidtdennis.challenges.leetcode.BFS doing stuff for each level without level delimiter
```
...
### 5) Find cycle in a de.schmidtdennis.challenges.leetcode.graph
...
The intuition behind Kahn's algorithm is to repeatedly remove nodes with indegree of zero from the de.schmidtdennis.challenges.leetcode.graph 
and add them to the topological ordering. We remove nodes with indegree zero from the de.schmidtdennis.challenges.leetcode.graph 
